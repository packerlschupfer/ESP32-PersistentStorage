/*
 * PersistentStorage.cpp - part of the ESP32-PersistentStorage library
 *
...
#include <algorithm>
#include <cstring>
#include <cmath>  // F24/F40: std::isfinite - reject NaN/Inf floats
...
PersistentStorage::Result PersistentStorage::loadParameter(ParameterInfo& param) {
    ...
    // F40: distinguish "key missing" from "loaded". Previously loadParameter
    // always returned SUCCESS, so loadAll()'s loadedCount always equalled the
    // parameter count and its first-boot autoSaveDefaults branch (loadedCount==0)
    // was dead. isKey() lets a genuinely-absent key keep the in-memory default
    // and be reported as ERROR_NOT_FOUND (not counted as loaded).
    ...
    switch (param.type) {
        case ParameterInfo::TYPE_BOOL: {
            *(bool*)param.dataPtr = preferences_.getBool(key.c_str(), *(bool*)param.dataPtr);
            break;
        }

        case ParameterInfo::TYPE_INT: {
            int32_t defaultVal = *(int32_t*)param.dataPtr;
            int32_t v = preferences_.getInt(key.c_str(), defaultVal);
            // F40: clamp/reject out-of-range NVS values (bit-rot, unit changes
            // across firmware versions, p%lu key-hash collisions) rather than
            // loading them unchecked into live safety settings - the MQTT set
...
PersistentStorage::Result PersistentStorage::jsonToParameter(ParameterInfo& param, const JsonDocument& doc) {
    ...
    switch (param.type) {
        ...
        case ParameterInfo::TYPE_FLOAT: {
            ...
            // F24: reject NaN/Inf. NaN passes BOTH range comparisons (NaN<min and
            // NaN>max are each false), so without the isfinite guard it was written
            // to the setting and persisted to NVS - poisoning PID gains / heating
            // curve params across reboots (e.g. "nan" via strtod on a plain payload).
            if (!std::isfinite(newVal) ||
                newVal < param.constraints.floatRange.min || newVal > param.constraints.floatRange.max) {
                return Result::ERROR_VALIDATION_FAILED;
            }
